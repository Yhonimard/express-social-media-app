  ...
  return useMutation(async (data) => {
    ...
    onSuccess: ({ data }, _var, context) => {
      queryClient.setQueryData([GET_POST_NAME, uid], (oldData) => {
        ...
        dispatch(globalReducer.action.showNotification({ message: "success create post", status: "success" }))
        return {
          ...oldData,
          pages
        }
    ...
    onError: (err, _var, context) => {
      ...
      dispatch(globalReducer.action.showLoadingOverlay(false))
      dispatch(globalReducer.action.showNotification({ message: message || "something went wrong, please try again to create post", status: "success" }))
