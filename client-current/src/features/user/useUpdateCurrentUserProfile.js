...
const useUpdateCurrentUserProfile = (uid) => {
  ...
  return useMutation(async (data) => {
    ...
    onError: (err, _var, context) => {
      ...
      dispatch(globalReducer.action.showNotification({ message: message || "error when updating profile, pls try again", status: "error" }))
      dispatch(globalReducer.action.showLoadingOverlay(false))

    },
    onSettled: () => {
      queryClient.invalidateQueries([GET_USER_PROFILE_NAME, uid])
      dispatch(globalReducer.action.showNotification({ message: "success updating profile" }))
      dispatch(globalReducer.action.showLoadingOverlay(false))
    }
  })
}
