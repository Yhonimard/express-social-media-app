  ...
  return useMutation(
    ...
    },
    {
      ...
      onError: (err, _var, context) => {
        if (!single)
          queryClient.setQueryData([GET_POST_NAME], context.prevData);
        if (single)
          queryClient.setQueryData([GET_POST_NAME, postId], context.prevData);

        const message =
          err?.response.data.message ||
          `hi ${currentUser.username}, something went wrong please try again to update this post`;
        dispatch(globalReducer.action.showLoadingOverlay(false));
        dispatch(
          globalReducer.action.showNotification({ message, status: "error" })
        );
      },
      onSettled: () => {
