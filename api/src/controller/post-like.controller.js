...
const PostLikeController = () => {
  ...
  const likePost = async (req, res, next) => {
    ...
    try {
      ...
      res.status(httpStatus.CREATED).json({ message: "success like post", data: response })
    } catch (error) {
      return next(error)
    }
  }
  ...
  const getUserHasLikeByCurrentUser = async (req, res, next) => {
    ...
    try {
      const response = await postLikeService.getUserHasLikeByCurrentUser(user, params?.pid)
