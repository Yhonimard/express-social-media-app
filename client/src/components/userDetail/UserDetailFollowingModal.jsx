...
const UserDetailFollowingCard = ({ username, photo_profile, isLoading, id, name }) => {
...
const UserDetailFollowingModal = ({ open, toggle, userId, username }) => {
  ...
  const followingQuery = friend.query.GetUserFollowing({ userId, search: searchValue })
    ...
    <Modal sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', px: 1 }} open={open} onClose={toggle}>
      <Paper sx={{ maxWidth: "30rem", width: "100%", p: 3 }}>
        <TextField
          variant="filled"
          size="small"
          label={`search ${username} following`}
          fullWidth
          margin="normal"
...
export default UserDetailFollowingModal;