...
const PostCard = ({ author, created_at, title, content, image, isFetching, pid }) => {
  ...
  const openCommentInput = useCallback((postId) => {
    dispatch(comment.reducer.action.openCommentInput({ postId }))
  }, [dispatch])

  const closeCommentInput = useCallback(() => {
    if (isOpenCommentInput)
      dispatch(comment.reducer.action.closeCommentInput())
    ...
    <>
      <Card>
        ...
        <CardActions>
          ...
          {(isOpenCommentInput || isOpenUpdateCommentInput) && (
            <IconButton onClick={closeCommentInput}>
              <Icon.Block />
            </IconButton>
          )}
          {(!isOpenCommentInput && !isOpenUpdateCommentInput) && (
            <IconButton onClick={() => openCommentInput(pid)}>
              <Icon.Comment />
            </IconButton>
          )}
        </CardActions>
        ...
        <Collapse in={isOpenCommentInput || isOpenUpdateCommentInput}>
          <CardActions>
            <CommentInput postId={pid} updateCommentState={isOpenUpdateCommentInput && updateCommentState} isUpdateComment={isOpenUpdateCommentInput} />
          </CardActions>
        </Collapse>

        <Box>
          <CommentList size={1} pid={pid} />
        </Box>
      </Card>
